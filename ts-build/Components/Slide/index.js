...
export class Slide extends Component {
    constructor(options) {
         ...
         * @returns {boolean}
         */
        this.slideFeedbackSwitch = (direction) => {
        ...
        this.whenSlideReachFeedbackTransformLimit = (transformDistance) => {
            const { region } = this.state;
            const { slideFeedbackTransformLimit, slideFeedback, } = this.props;
            const first = region[0];
            const last = region[region.length - 1];
            const howFarFromBeginToCurrent = transformDistance - first, howFarFromEndToCurrent = last - transformDistance;
        ...
        this.binaryChop = ({ region, target, direction }) => {
            //此处region由大到小排列
            let left = 0, right = region.length - 1, mid = Math.floor((left + right) / 2);
        ...
        this.getLayout = (instance) => {
            if (!instance)
                return;
            const { children, layout, defaultActiveIndex, } = this.props;
            const count = React.Children.count(children);
            const width = instance.offsetWidth, height = instance.offsetHeight;
    ...
    render() {
      ...
      <div className={`slide-collection`} style={styleOfSlideCollection} onTouchStart={this.start} onTouchMove={this.move} onTouchEnd={this.end} onTouchCancel={this.end}>
        {this.getSlidePane()}
      </div>
    </div>;
    }
