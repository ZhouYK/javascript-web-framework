...
const PULLTEXT = '下拉刷新', RELEASETEXT = '释放', LOADING = '正在加载';
class PullRefresh extends Component {
    constructor(options) {
        ...
        this.move = (evt) => {
            ...
            if (this.switchRate <= 0.7 && this.scrollTop <= 0) {
                ...
                let transformDistance = this.cacheTransform + currentMove, text = _text;
                //滑动到上限的一半时变化
        ...
        this.end = (evt) => {
            ...
            let finalMove = transform, loading = false, text = _text;
        ...
        this.getRequestResult = ({ loading, text, }) => {
            this.setState({
                loading,
                text
            }, () => {
    ...
    render() {
        ...
        return <section className="pull-refresh" onTouchStart={this.start} onTouchMove={this.move} onTouchEnd={this.end} onTouchCancel={this.end}>
      <section className="pull-refresh-loading-icon" style={iconStyle}>
        {text}
      </section>
       <section style={style} className="pull-refresh-inner" onTransitionEnd={this.handleTransitionEnd}>
         <section className="pull-refresh-content">
           {children}
         </section>
       </section>
    </section>;
