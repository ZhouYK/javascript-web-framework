...
class ItemMove extends Component {
    constructor(options) {
        ...
        this.preventWhenSlideToRight = (direction) => {
            const { transform } = this.state;
            const startPoint = 0, endPoint = this.calculateButtonsWidth();
            if (direction === 1) {
        ...
        this.move = (event) => {
            ...
            if (this.switchRate <= 0.7) {
                ...
                let finalTransformDistance, startPoint = 0, endPoint = this.calculateButtonsWidth();
        ...
        this.getButtons = () => {
            ...
            return buttons.map((button, index) => {
                return <WidgetButton onClick={button.onClick} width={button.width} className={`widget-button-${button.type}`} key={`widget-button-${index}`}>
        {button.text}
      </WidgetButton>;
            });
        };
    ...
    render() {
      ...
      <div className="item-move-content" style={styleOfContent} onTouchStart={this.start} onTouchMove={this.move} onTouchEnd={this.end} onTouchCancel={this.end}>
        {children}
      </div>
